...
class LSTimingStrategy(Strategy):
    ...
    def should_long(self) -> bool:
        ...
        return calculate_percent(self.close, self.long_ema) < -PIKE_PERCENT and utils.strictly_increasing(self.candles[1], NB_CONSEQ)
    ...
    def should_short(self) -> bool:
        ...
        return calculate_percent(self.close, self.long_ema) > PIKE_PERCENT and utils.strictly_decreasing(self.candles[1], NB_CONSEQ)

    def should_cancel(self) -> bool:
        return True

    def go_long(self):
        qty = utils.size_to_qty(self.capital * CAPITAL_PERCENT, self.price, fee_rate=self.fee_rate)
    ...
    def update_position(self):
        if self.is_long:
            ...
            if self.close > self.current_pyramiding_levels  * (1 + 3* UPDATE_PERCENT):
                self.current_pyramiding_levels = self.current_pyramiding_levels + ( self.current_pyramiding_levels  * UPDATE_PERCENT)
