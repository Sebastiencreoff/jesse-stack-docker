...
class micro_strategy(Strategy):
    def should_long(self) -> bool:
        if self.bullish: 
            #if close is lower than 10% of the lowerband
            if 0 < (self.close / self.bb.lowerband[-1]) *100 - 100 < 10:
                return True
    ...
    def should_short(self) -> bool:
        if self.bearish: 
            #if close is lower than 10% of the lowerband
            if 0 < (self.bb.upperband[-1] / self.close) *100 - 100 < 10:
                return True
    ...
    def update_position(self):
        if self.is_long:
            if self.close > self.average_entry_price + (self.average_entry_price * 0.02):
                self.stop_loss = self.position.qty, self.average_entry_price + (self.average_entry_price * 0.02)
        if self.is_short:
            if self.close < self.average_entry_price - (self.average_entry_price * 0.02):
                self.stop_loss = self.position.qty, self.average_entry_price - (self.average_entry_price * 0.02)
    ...
    def hyperparameters(self):
        return [
            {'name': 'take_profit_value', 'type': int, 'min': 1, 'max': 100, 'default': 10},
            {'name': 'stop_loss_value', 'type': int, 'min': 1, 'max': 100, 'default': 10},
        ]